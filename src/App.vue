<template>
	...
	<div class="sm:px-0 mx-auto mt-12 w-full  ">
    ...
    {{ userWallet !== null ? 'CONNECTED' : 'NOT CONNECTED' }}
		...
		<div class="flex lg:flex-row flex-col pb-4 lg:space-x-4 p- bg-gray-100 mt-4 border-4 border-gray-200 rounded-lg">
			<div class="lg:w-[39%] ">
				<p class="font-bold text-2xl text-center">STAKE OR WITHDRAW</p>
				<div class="flex ml-4 flex-col py-4 px-6 border-gray-300 bg-[#E6E6E6] border-2 rounded-2xl  pt-10 lg:h-[35.6rem]">
					<div class="flex lg:flex-row lg:text-xl  mx-auto items-center lg:space-x-3">
						...
						<div
							...
							{{ lockAPRsFrontTotal !== undefined && lockAPRsFrontTotal !== 0 ? `${lockAPRsFrontTotal}%` : `7% ` }} APR
							<br /> TOTAL
						</div>
					</div>
					<div class="relative flex items-center w-full  mb-4 mt-4 border-2 bg-white p-4 border-gray-400 rounded-lg">
						...
						<input type="text"
							class=" px-3 py-2 text-xl focus:outline-none font-bold focus:ring-2 focus:ring-blue-500 w-full"
...
console.log(this.fullBalanceOfToken)
				let get_userLock = await stakingContract.methods.userLock(this.userWallet).call();
				console.log(get_userLock)
				console.log(get_userLock['bonusBalance'])
